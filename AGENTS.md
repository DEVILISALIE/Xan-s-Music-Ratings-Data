...
- **版本号**：`package.json` / `src-tauri/Cargo.toml` / `src-tauri/tauri.conf.json` 当前为 `1.5.0`；标题栏通过 `get_app_version` 读取 package info；About 弹窗文案也写为 `v1.5.0`

**macOS 原生鼠标光标体系（v1.5.0）**：应用内统一使用纯矢量 macOS 原生风格光标：
- `--cursor-default`：macOS 经典白边黑底箭头（带柔和投影）
- `--cursor-pointer`：macOS 圆润白手套食指指针（作用于所有按钮、卡片、标签、下拉项、开关等交互元素）
- `--cursor-text`：macOS 经典带上下双向衬线挡角与黑白高对比边缘的工字钢 I-Beam 光标（作用于所有输入框、数值框、搜索框与编辑区）
- `--cursor-grab` / `--cursor-grabbing`：macOS 原生开掌与闭合抓手光标（作用于图片查看器拖拽与平移）

**设计工程动效体系（v1.5.0）**：遵循 Emil Kowalski / Apple 设计工程体系：
- 统计面板：7 档分数分布柱状图引入 30ms 阶梯级联延展动画（`barGrow`）；单行悬停（Row Exploration）支持背景高亮、柱状条增亮 15%、计数位移；卡片悬停等比放大（`scale(1.03)`）与按压下压回弹（`scale(0.97)`）
- 年度统计弹窗：去除非必要的横向滚动条，重构盒模型（`overflow-x: hidden` + `box-sizing: border-box`）自适应宽度
- 弹窗防闪烁：移除进场动画容器内部子表单控件的 `backdrop-filter`，防止 GPU FBO 隔离导致的表单纯白闪烁；统一曲目序号、封面操作按钮与输入框质感底色
- 搜索快捷键：在搜索模式下支持键盘 ↑ / ↓ 方向键在搜索结果间循环跳转
